...
export const NETWORK = process.env.NEXT_PUBLIC_NETWORK ?? "studionet";
...
function requireContractAddress(): string {
  if (!CONTRACT_ADDRESS) {
    throw new GenLayerClientError(
      "NEXT_PUBLIC_CONTRACT_ADDRESS is not configured. Copy frontend/.env.example to .env.local and set it after deploying the contract."
    );
  }
